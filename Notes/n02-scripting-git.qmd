...
First make a a new directory called `scripts` and navigate to it. And make a file called `time.sh`. In the file put this command:
``` bash
echo $(date)
```
`$(date)` is a value maintained by the system and accessible from the command line.
In order to run the is script we run the command `./time.sh` however at the moment that will give us this output:
``` bash
$ ./time.sh
