...
# Shell scripts
Shell scripts, often called bash scripts due to the prevalence of the bash script, are text file containing terminal commands that can be run to run all the commands. It is typical but not required in linux to give these files the .sh file extension. These are usually used to automate simple tasks,  though shell scripting is essentially a fully featured programming language.
...
* The `echo` command simply outputs text to the command line.
* `$(date)` passes the output of the `date` command to be part of the string passed to the echo command. This is one of many strange shortcuts in shell scripting.
...
# Arguments
Most of the commands that we have interacted with so far expect some command line arguments when they are run. We too can make our shell scripts more versatile or reusable by making them accept arguments. 

Within the script arguments can be accessed as numbered variables. i.e. `$0` will hold the value of the command itself and subsequent values (`$1``$2` etc...) will contain the arguments that have been passed. The number of arguments is stored in the `$#` variable

``` bash
#!/bin/bash

echo "This program was passed $# arguments"
echo "The program command was $0"
echo "The first argument was $1"
echo "The last argument was ${@:$#}"
```
:::{.callout-note}
The last line uses a special notation to access the `$#`th element of `$@` which is the array of arguments. It is not necessary to understand this.
:::