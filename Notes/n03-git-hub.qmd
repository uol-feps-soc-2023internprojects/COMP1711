...
# What are branches?
You may have noticed branches mentioned when using `git status` that you are on the main branch.

A git branch is a pointer to a specific commit, when you are on a branch as you continue to commit and add to this branch this pointer will be updated to point to the most recent branch. 
...
#  What is origin?
Origin is shorthand for the remote storage location of the repository. In our case this is on Github. if we run `git status` we should see that "Your branch is ahead of 'origin/main' by 1 commit."
...
```
Following the URL will open your github project page, this is setup by Codespaces automatically in this scenario. In this case Github is simply a remote location to store a copy of the repository. 

To update this remote we use the command `git push`. (This branch is setup to to be linked to the remote main, if that were not the case we would have to specify where to push to)

Once this is done, the `git status` to read "Your branch is up to date with 'origin/main'."
